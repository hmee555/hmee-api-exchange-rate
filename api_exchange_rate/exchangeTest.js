		...
		if (!error && response.statusCode == 200) {
			...
			if(strJson.result.data.data_detail[0].buying_transfer != "" &&       
			   ...
			   strJson.result.data.data_detail[0].mid_rate != ""){
				...
				fs.appendFile(/*filename*/fullPath, strFinal, /*{mode: parseInt('0755', 8)} ,*/ function (err) {  // writeFile = overwrite
				if (err) throw err;
					//console.log('Saved!');		
				else
				...
				fs.writeFile(ctrlPath, strNull, /*{mode: parseInt('0755', 8)} , */function (err) { 
				if (err) throw err;	
				else
